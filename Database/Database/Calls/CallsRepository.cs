...
public class CallsRepository : ICallsRepository, IDisposable
{
	...
	private Task PutMessages(CallsContext ctx, IList<(Data, Call)> calls)
	{
		...
		return ctx.AddRangeAsync(messages);
	}

	private Task PutHeaders(CallsContext ctx, IList<(Data, Call)> calls)
	{
		...
		return ctx.StoredProcedure($"insert_headers", headers);
	}
